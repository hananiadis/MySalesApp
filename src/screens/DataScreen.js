...
import {
  saveProductsToLocal, clearProductsLocal, getProductsLastAction,
  saveCustomersToLocal, clearCustomersLocal, getCustomersLastAction,
  getImagesLastAction, clearProductImagesCache
...
const DataScreen = () => {
  ...
  const [loading, setLoading] = useState({
    products: false, customers: false, prodClear: false, custClear: false, images: false, imgClear: false
  });
  ...
  useEffect(() => { refreshActions(); }, []);
    ...
    try {
      ...
      const normalizedProducts = Array.isArray(products)
        ? products.map((prod = {}) => {
            if (!stockMap || typeof stockMap.get !== 'function') return prod;
            const value = lookupImmediateStockValue(stockMap, prod?.productCode);
            if (value != null && value !== '') {
              return { ...prod, availableStock: value };
            }
            return { ...prod, availableStock: 'n/a' };
          })
        : products;
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={styles.button} onPress={handleDownloadProducts} disabled={loading.products}>
            {loading.products ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Λήψη από Firestore</Text>}
          </TouchableOpacity>
          <TouchableOpacity style={[styles.button, styles.clearButton]} onPress={handleClearProducts} disabled={loading.prodClear}>
            {loading.prodClear ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Διαγραφή τοπικών</Text>}
          </TouchableOpacity>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={styles.button} onPress={handleDownloadCustomers} disabled={loading.customers}>
            {loading.customers ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Λήψη από Firestore</Text>}
          </TouchableOpacity>
          <TouchableOpacity style={[styles.button, styles.clearButton]} onPress={handleClearCustomers} disabled={loading.custClear}>
            {loading.custClear ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Διαγραφή τοπικών</Text>}
          </TouchableOpacity>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={styles.button} onPress={handleDownloadImages} disabled={loading.images}>
            {loading.images ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Λήψη Εικόνων</Text>}
          </TouchableOpacity>
          <TouchableOpacity style={[styles.button, styles.clearButton]} onPress={handleClearImages} disabled={loading.imgClear}>
            {loading.imgClear ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Διαγραφή Εικόνων</Text>}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  section: { marginBottom: 32, padding: 16, backgroundColor: '#F8F8F8', borderRadius: 8, elevation: 2 },
  sectionTitle: { fontSize: 20, fontWeight: '600', marginBottom: 10, color: '#007AFF' },
  button: { backgroundColor: '#007AFF', borderRadius: 6, padding: 12, marginVertical: 6, alignItems: 'center' },
