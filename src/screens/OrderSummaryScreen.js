...
const PAYMENT_LABELS = {
  prepaid_cash: "\u03a0\u03c1\u03bf\u03b5\u03af\u03c3\u03c0\u03c1\u03b1\u03be\u03b7 \u0028\u03ad\u03ba\u03c4\u03c0\u03c9\u03c3\u03b7 \u0033\u0025\u0029",
...
const OrderSummaryScreen = ({ navigation }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const { uri, mime, fileName } = await exportOrderAsXLSX(exportPayload);
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <Row label={STRINGS.name} value={customer?.name || customer?.displayName || ""} />
        <Row label={STRINGS.vat} value={getVat()} />
        <Row label={STRINGS.phone} value={getPhone()} />
        <Row label={STRINGS.address} value={customer?.address?.street ?? customer?.address ?? ""} />
        <Row label={STRINGS.city} value={customer?.address?.city ?? ""} />
        <Row label={STRINGS.customerCode} value={customer?.customerCode ?? customer?.code ?? ""} />
        ...
        <Row label={STRINGS.discount} value={formatEuro(discount)} />
