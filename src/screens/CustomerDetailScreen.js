...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Linking, Alert, SafeAreaView, ActivityIndicator, Image } from 'react-native';
...
const CustomerDetailScreen = ({ route, navigation }) => {
  ...
  if (loading) {
    return <SafeAreaView style={styles.container}><ActivityIndicator color="#007AFF" size="large" /></SafeAreaView>;
  }
  if (!customer) {
    return <SafeAreaView style={styles.container}><Text style={styles.error}>Πελάτης δεν βρέθηκε.</Text></SafeAreaView>;
  }
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollInner}>
        <View style={styles.headerSection}>
          <Image
            source={require('../../assets/customer_avatar.png')}
            style={styles.avatar}
            resizeMode="cover"
          />
          <View style={{ flex: 1 }}>
            <Text style={styles.headerTitle}>{customerCode} - {name}</Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('CustomerSalesSummary', { customerId })}
        >
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    marginRight: 10, backgroundColor: '#e0e0e0',
  },
